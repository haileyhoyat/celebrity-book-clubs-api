...
async function get_all_books(){
    await axios.get('https://reesesbookclub.com/article/4eRlfCOXueqPrm6ZnQpzwl')
        .then((response)=>{
    ...
    await axios.get('https://www.today.com/shop/read-jenna-book-club-list-today-s-jenna-bush-hager-t164652')
        .then((response)=>{
            const html = response.data
            const $ = cheerio.load(html)
    ...
    await axios.get('https://www.goodmorningamerica.com/culture/story/shop-gma-book-club-picks-list--81520726')
        .then((response) => {
            const html = response.data
            const $ = cheerio.load(html)
            all_items = []
